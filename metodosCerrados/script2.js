// Muestra todas las iteraciones del metodo falsa posicion
...
document.getElementById("formulario").addEventListener("submit", function (e) {
  ...
  for (let i = 1; i <= iterMax; i++) {
    if (metodo === "biseccion") {
      ...
      puntosXR.push({ x: xr, y: evaluarFuncion(funcionStr, xr) });
    }
  ...
  actualizarGeoGebra(funcionStr, metodo, puntosXR);
});
...
function actualizarGeoGebra(funcionStr, metodo, puntosXR) {
  const funcionGG = convertirFuncionAGeoGebra(funcionStr);
  let comandos = [`f(x) = ${funcionGG}`];

  if (metodo === "falsa") {
    puntosXR.forEach((p, i) => {
      comandos.push(`A${i + 1} = (${p.x.toFixed(6)}, ${p.y.toFixed(6)})`);
    });
  }

  const scriptComandos = comandos.map(cmd => `api.evalCommand("${cmd}");`).join("\n");
